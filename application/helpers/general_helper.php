...
if (!function_exists('get_estimate_making_data')) {
    ...
    function get_estimate_making_data($estimate_id) {
        ...
        if ($estimate_info) {
            ...
            $data['division_types'] = $ci->Division_model->get_division_types_active()->result();
            $data['estimates_divisions'] = $ci->Estimate_division_model->get_estimate_division_active($estimate_id)->result();
            $data["estimate_total_summary"] = $ci->Estimates_model->get_estimate_total_summary($estimate_id);
            if($data['estimate_info']->project_id !== 0){ //Se agregan los datos del proyecto asociado.
                $data["project_info"] = $ci->Projects_model->get_one($data['estimate_info']->project_id);
            }

            $data["all_total"] = 0;
            $data["total_division"] = array();
            if(isset($data["division_types"]) && $data["division_types"]){
                foreach($data["division_types"] as $division){
                    $total = 0;
                    foreach($data["estimates_divisions"] as $estimate){
                        if($division->division_id == $estimate->division_id){
                            $total += $estimate->total;
                        }
                    }
                    $data["all_total"] += $total;
                    array_push($data["total_division"], $total);
                }
            }
